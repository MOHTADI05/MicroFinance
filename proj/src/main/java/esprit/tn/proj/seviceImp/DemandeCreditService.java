...
import esprit.tn.proj.Controller.DemandeCreditController;
import esprit.tn.proj.entity.*;
import esprit.tn.proj.repository.*;
    ...
    public DemandeCredit add(Long existingCreditId, Long Id_fund,Long Id_client,Long Id_garantor, float year, TypeCredit typeC, float amount,LocalDate monthlyPaymentDate) {
        ...
        switch (typeC) {
            ...
            for (int i = 0; i < year - 1; i++) {
                ...
                amortissements.add(new Amortissement(remainingAmountAffine, monthlyPaymentI, 0, monthlyPaymentI));
            }
            // Dernière année, payer le montant total avec les intérêts
            amortissements.add(new Amortissement(amount, monthlyPaymentI,amount , remainingAmountAffine));
            ...
            case AMORTISSEMENT_CONSTANT:
                ...
                for (int i = 0; i < year * 12; i++) {
                    ...
                    amortissementss.add(new Amortissement(remainingAmount, interest, amortissement, monthlyPaymentA));
        ...
        //EXISTING CLIENT
            ...
            float Ratio_retard = (CaculateLateDays(demaneCreditRepository.findTopByClientIdAndCompletedIsTrueAndStateIsTrueOrderByObtainingdateDesc(Id_client))) / (demaneCreditRepository.findTopByClientIdAndCompletedIsTrueAndStateIsTrueOrderByObtainingdateDesc(Id_client).getYear() * 12 * 30);
            //3 CAS
            if (Ratio_retard < 0.1) {
