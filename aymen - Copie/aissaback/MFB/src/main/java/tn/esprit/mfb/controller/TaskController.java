...
//    }
    ...
    public ResponseEntity<String> addTaskProgress(@PathVariable Long taskId, @RequestBody double progressToAdd) {
        try {
            taskService.addTprogress(taskId, progressToAdd);
            return ResponseEntity.ok("Progress added successfully.");
        } catch (IllegalArgumentException e) {
            return ResponseEntity.badRequest().body(e.getMessage());
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("An error occurred while adding progress to the task.");
        }
    }
