...
public class ComplaintController {
    ...
    @PutMapping("/{complaintId}/attach")
    public ResponseEntity<String> attachFileToComplaint(@PathVariable("complaintId") Integer complaintId,
                                                        @RequestParam("file") MultipartFile file) {
        complaintService.attachFile(complaintId, file);
        return ResponseEntity.ok("File attached successfully.");
    }

    @GetMapping("/download/{complaintId}")
    public ResponseEntity<byte[]> downloadFileFromComplaint(@PathVariable("complaintId") Integer complaintId) throws IOException {
        byte[] fileBytes = StreamUtils.copyToByteArray(complaintService.downloadFile(complaintId).getInputStream());

        return ResponseEntity.ok()
                .contentType(MediaType.APPLICATION_OCTET_STREAM)
                .contentLength(fileBytes.length)
                .header("Content-Disposition", "attachment; filename=complaint_attachment")
                .body(fileBytes);
    }
